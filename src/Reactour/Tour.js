...
class Tour extends Component {
  constructor () {
    ...
    EventDispatcher.register('tourSuspend', this.suspend, this)
  }

  suspend (evtName, { interval }) {
    this.setState({ suspend: true })
    setTimeout(() => this.setState({ suspend: false }), interval)
  }
  ...
  componentWillUnmount () {
    EventDispatcher.unregister('tourSuspend', this.suspend)
  ...
  open (startAt) {
    ...
    this.setState(
      prevState => ({
        isOpen: true,
        current: startAt !== undefined ? startAt : 0 // prevState.current
      }),
      () => {
  ...
  calculateNode = (node, stepPosition, cb) => {
    ...
    if (!hx.inView({ ...attrs, w, h, threshold: inViewThreshold })) {
      ...
      const offset = scrollOffset || (attrs.height > h
        ? -25
        : -(h / 2) + attrs.height / 2)
      ...
      return (
        <Portal>
          ...
          <SvgMask
            ...
            rounded={rounded}
            onSpotClick={
              steps[current].onSpotClick
                ? () => steps[current].onSpotClick(this.gotoStep, dispatch, node)
                : () => {}
            }
            disableInteraction={
